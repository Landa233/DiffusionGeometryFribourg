...
def test_circular_coordinates_smoke_on_circle():
    ...
    assert np.isfinite(result.candidate.dirichlet_energy)
    assert result.candidate.dirichlet_energy > 0.0
...
def test_circular_coordinates_accepts_reweighted_hodge_up_term():
    ...
    result = circular_coordinates(dg, epsilon=1.0, k=6, hodge_up_weight=0.25)

    assert result.angle.shape == (dg.n,)
    assert len(result.candidates) > 0
    assert np.isfinite(result.candidate.hodge_eigenvalue)
